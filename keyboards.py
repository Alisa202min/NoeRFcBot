...
def product_categories_keyboard(categories: List[Dict]) -> InlineKeyboardMarkup:
    ...
    for category in categories:
        ...
        callback_data = f"{PRODUCT_PREFIX}cat_{category_id}"
        try:
...
def service_categories_keyboard(categories: List[Dict]) -> InlineKeyboardMarkup:
    ...
    for category in categories:
        category_name = category['name']
        category_id = category['id']
        callback_data = f"{SERVICE_PREFIX}cat_{category_id}"
        try:
...
def product_content_keyboard(products: List[Dict], category_id: int) -> InlineKeyboardMarkup:
    ...
    for product in products:
        builder.button(text=product['name'], callback_data=f"{PRODUCT_PREFIX}:{product['id']}")
    builder.button(text=BACK_BTN, callback_data=f"{PRODUCT_PREFIX}cat_{category_id}")
...
def service_content_keyboard(services: List[Dict], category_id: int) -> InlineKeyboardMarkup:
    ...
    for service in services:
        builder.button(text=service['name'], callback_data=f"{SERVICE_PREFIX}:{service['id']}")
    builder.button(text=BACK_BTN, callback_data=f"{SERVICE_PREFIX}cat_{category_id}")
    builder.button(text="🏠 بازگشت به منوی اصلی", callback_data="back_to_main")
    builder.adjust(1)
    return builder.as_markup()
...
def product_detail_keyboard(product_id: int, category_id: int) -> InlineKeyboardMarkup:
    ...
    """
    keyboard = [
        [InlineKeyboardButton(text="استعلام قیمت 📝", callback_data=f"{INQUIRY_PREFIX}product:{product_id}")],
        [InlineKeyboardButton(text=BACK_BTN, callback_data=f"{PRODUCT_PREFIX}cat_{category_id}")],
...
def service_detail_keyboard(service_id: int, category_id: int) -> InlineKeyboardMarkup:
    ...
    """
    keyboard = [
        [InlineKeyboardButton(text="استعلام قیمت 📝", callback_data=f"{INQUIRY_PREFIX}service:{service_id}")],
        [InlineKeyboardButton(text=BACK_BTN, callback_data=f"{SERVICE_PREFIX}cat_{category_id}")],
        [InlineKeyboardButton(text="🏠 بازگشت به منوی اصلی", callback_data="back_to_main")]
    ]
    return InlineKeyboardMarkup(inline_keyboard=keyboard)
...
# Other keyboard functions (admin, cancel, etc.) remain unchanged for brevity
