...
if [ "$USE_GIT" = "y" ] || [ "$USE_GIT" = "Y" ]; then
    read -p "آدرس مخزن گیت (مثال: username/rfcbot یا 
    if [ -z "$REPO_URL" ]; then
        print_message "آدرس مخزن وارد نشده. استفاده از مقدار پیش‌فرض: RFcBotUsr/NoeRFcBot"
        
    fi
    # Normalize REPO_URL (remove https://github.com/ or .git if present)
    REPO_URL=$(echo "$REPO_URL" | sed 's|https://github.com/||; s|\.git$||')
    read -p "شاخه مخزن (پیش‌فرض: main): " GIT_BRANCH
